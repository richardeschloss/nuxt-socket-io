...
export const register = {
  ...
  async serverAPI ({
    ...
    const fetchedApi = await state.emit({
      label: apiLabel,
      socket,
      ...
      store.watch(
        (state) => {
          ...
          if (watchProp === undefined) {
            // throw new Error(
            console.error( // TBD: no need to halt...just communicate error
              [
  ...
  iox ({ stateOpts, socket }) {
    console.log('register.iox', stateOpts)
    ...
    stateOpts.forEach((entry) => {
      ...
      if (toRegex.test(entry)) {
        console.log('entry ONE way TO state', entry)
        socket.on(evt, (msg) => {
          console.log('evt rxd!', evt, msg)
          if (nsp) {
            iox[nsp][dest] = msg
          } else {
            iox[dest] = msg
        ...
        console.log('entry ONE way FROM state', entry)
      } else {
        console.log('bidrectional', entry)
      }
      // console.log('evt, dest', evt, dest)
    })
  },
  ...
  stubs (ctx) {
    ...
    if (!ctx.$on) { //  || !ctx.$emit || !ctx.$once) {
      ...
      ctx.$$emit = (...args) => emitter.emit(...args) // TBD: replace ctx.$emit with ctx.$$emit calls
    }
    ...
    if (!ctx.$watch) { // TBD: seems to be defined in Nuxt3 ?
      ctx.$watch = (label, cb) => {
...
function nuxtSocket (ioOpts) {
  ...
  if (stateOpts) {
    register.iox({ stateOpts, socket })
    // console.log('register', stateOpts, ioState())
    // const ioS = ioState().value
    // ioS.xyz = {} // reactive({})
    // watch(() => ioS.xyz, (n, o) => {
    //   console.log('xyz changed!', n)
    // })
  }
  // socket.on('chatMessage', (msg) => {
  //   const ioS = ioState().value
  //   console.log('chatMessage rxd!', msg, ioS)
  //   ioS.chats = {
  //     message: msg
  //   }
  // })
